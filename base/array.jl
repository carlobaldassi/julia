...
# TODO: use functions instead of macros as soon as performance
#       of functions passed as arguments improves
macro _jl_compare_array_to_array(isf, A, B)
    F = gensym()
    quote
...
macro _jl_compare_scalar_to_array(isf, A, B)
    F = gensym()
    quote
        $F = similar($B, Bool)
        for i = 1:numel($B)
...
macro _jl_compare_array_to_scalar(isf, A, B)
    F = gensym()
    quote
        $F = similar($A, Bool)
        for i = 1:numel($A)
...
#       etc.
#       (and then add specialized String versions, similar to those for Numbers)
#
