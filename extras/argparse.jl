...
# aux functions (some shoud get into stdlib?)
#{{{
deep_copy(x::String) = copy(x)
...
rope_string() = RopeString("")
rope_string(s::String) = RopeString(s)
rope_string(s::String, t::RopeString) = RopeString(s, t)
function rope_string(s::String...)
    ind = div(length(s), 2)
    return RopeString(rope_string(s[1:ind]...), rope_string(s[ind+1:end]...))
end
...
type ArgParserField
    ...
    function ArgParserField()
        return new("", String[], String[], Any, store_true, ArgConsumerType(), nothing, nothing, x->true, false, "", "")
    end
end
...
#{{{
_check_all_keys_are_valid(desc::Dict, valid_keys) = _check_all_keys_are_valid(desc, valid_keys, false)
function _check_all_keys_are_valid(desc::Dict, valid_keys, warn::Bool)
    for k in keys(desc)
        found = false
        ...
        if !found
            if warn
                println(stderr_stream, "warning: ignored key: $k")
            else
                error("invalid key: $k")
            end
        end
...
function _check_default_type(default, arg_type::Type)
    if !is(default, nothing) && !isa(default, arg_type)
...
function _check_default_type_multi(default, arg_type::Type)
    if !is(default, nothing) && !isa(default, Vector{None}) && !isa(default, Vector{arg_type})
...
function _check_default_type_multi2(default, arg_type::Type)
    if !is(default, nothing) && !isa(default, Vector{None}) &&
...
add_argument(arg_parser::ArgumentParser, name::ArgName) = add_argument(arg_parser, name, Dict{String,Any}())
function add_argument(arg_parser::ArgumentParser, name::ArgName, desc::Dict)
    _check_name_format(name)
    
    valid_keys = ["name", "nargs", "action", "arg_type", "default", "const", "is_mandatory", "range_tester", "dest_name", "help", "metavar"]
    _check_all_keys_are_valid(desc, valid_keys)

    if !has(desc, "nargs")
        nargs = ArgConsumerType()
    else
        nargs = ArgConsumerType(desc["nargs"])
    end
    if !has(desc, "action")
    ...
    if is_flag
        ...
        if action == store_const || action == append_const
            push(valid_keys, "const", "arg_type", "dest_name")
        ...
        _check_all_keys_are_valid(desc, valid_keys, true)

        if (action == store_const || action == append_const)
            if !has(desc, "const")
                error("action $action requires the 'const' field")
            end
        end
    elseif is_opt
        valid_keys = ["nargs", "action", "arg_type", "default", "range_tester", "dest_name", "help", "metavar"]
        if nargs.desc == '?'
            push(valid_keys, "const")
        end
        _check_all_keys_are_valid(desc, valid_keys, true)
    else
    ...
    if has(desc, "dest_name"); new_arg.help = desc["dest_name"]; end
    if has(desc, "arg_type"); new_arg.arg_type = desc["arg_type"]; end
    if has(desc, "default"); new_arg.default = deep_copy(desc["default"]); end
    if has(desc, "const"); new_arg.constant = deep_copy(desc["const"]); end
    if has(desc, "range_tester"); new_arg.range_tester = deep_copy(desc["range_tester"]); end
    if has(desc, "is_mandatory"); new_arg.is_mandatory = desc["is_mandatory"]; end
    if has(desc, "help"); new_arg.help = desc["help"]; end
    if has(desc, "metavar"); new_arg.metavar = desc["metavar"]; end
    ...
    if is_flag
        ...
        if (is_multi_action(new_arg.action) && is_multi_nargs(new_arg.nargs)) && (is(default, nothing) || default == [])
            new_arg.default = Array(Vector{arg_type}, 0)
        elseif (is_multi_action(new_arg.action) || is_multi_nargs(new_arg.nargs)) && (is(default, nothing) || default == [])
...
function _build_usage_line(arg_parser::ArgumentParser)
    ...
    for f in arg_parser.args
        if is_arg(f)
            ...
            if f.nargs.desc == 'N'
                arg_str = rope_string(ntuple(f.nargs.num, i->(i==1?f.metavar:rope_string("\ua0", f.metavar)))...)
            elseif f.nargs.desc == 'A'
                arg_str = f.metavar
            elseif f.nargs.desc == '?'
                _found_a_bug()
            elseif f.nargs.desc == '*' || f.nargs.desc == 'R' || f.nargs.desc == '+'
                arg_str = rope_string(f.metavar, "...")
            else
                _found_a_bug()
            end
            push(pos_lst, rope_string(bra_pre, arg_str, bra_post))
        else
            if !isempty(f.short_opt_name)
                opt_str1 = rope_string("-", f.short_opt_name[1])
            else
                opt_str1 = rope_string("--", f.long_opt_name[1])
            end
            if is_flag(f)
                opt_str2 = ""
            else
                if f.nargs.desc == 'N'
                    opt_str2 = rope_string(ntuple(f.nargs.num, i->rope_string("\ua0", f.metavar))...)
                elseif f.nargs.desc == 'A'
                    opt_str2 = rope_string("\ua0", f.metavar)
                elseif f.nargs.desc == '?'
                    opt_str2 = rope_string("\ua0[", f.metavar, "]")
                elseif f.nargs.desc == '*' || f.nargs.desc == 'R'
                    opt_str2 = rope_string("\ua0[", f.metavar, "...]")
                elseif f.nargs.desc == '+'
                    opt_str2 = rope_string("\ua0", f.metavar, "\ua0[", f.metavar, "...]")
                else
                    _found_a_bug()
                end
            end
            new_opt = rope_string("[", opt_str1, opt_str2, "]")
    ...
    if isempty(opt_lst)
        ...
        optl_str = rope_string(ntuple(length(opt_lst), i->(i==1?opt_lst[1]:rope_string(" ", opt_lst[i])))...)
    end
    if isempty(pos_lst)
        posl_str = ""
    else
        posl_str = rope_string(ntuple(length(pos_lst), i->(i==1?pos_lst[1]:rope_string(" ", pos_lst[i])))...)
    end
    ...
    str_nonwrapped = rope_string(usage_pre, " ", optl_str, " ", posl_str)
    str_wrapped = TextWrap.wrapped_string(str_nonwrapped, twopts)

    out_str = replace(str_wrapped, '\ua0', ' ')
    return out_str
end
...
function _gen_help_text(arg::ArgParserField)
    ...
    if !is_flag(arg)
        ...
        const_str = ""
        if arg.arg_type != Any
            type_str = rope_string(pre, "(type: ", string(arg.arg_type))
        end
        if !is(arg.default, nothing) && !isequal(arg.default, [])
            mid = isempty(type_str) ? " (" : ", "
            default_str = rope_string(mid, "default: ", string(arg.default))
        end
        if arg.nargs.desc == '?'
            mid = isempty(type_str) && isempty(default_str) ? " (" : ", "
            const_str = rope_string(mid, "without arg: ", string(arg.constant))
        end
        post = (isempty(type_str) && isempty(default_str) && isempty(const_str)) ? "" : ")"
        return rope_string(arg.help, type_str, default_str, const_str, post) 
    else
        return arg.help
    end
end
...
function _show_help(arg_parser::ArgumentParser)
    ...
    for f in arg_parser.args
        if is_arg(f)
            ...
            new_opt = {rope_string(opt_str1, opt_str2), _gen_help_text(f)}
            push(opt_lst, new_opt)
    ...
    sindent = rope_string(lmargin, " " ^ lc_len, rmargin)
    ...
    if length(arg_parser.description) > 0
        TextWrap.println_wrapped(arg_parser.description, twopts_desc)
        println()
    end
    if length(pos_lst) > 0
        ...
        for l in pos_lst
            ...
            if l1len <= lc_usable_len
                rfill = " " ^ (lc_len - l1len)
                ll_nonwrapped = rope_string(l[1], rfill, rmargin, l[2])
                ll_wrapped = TextWrap.wrapped_string(ll_nonwrapped, twopts_block1)
                println(replace(ll_wrapped, '\ua0', ' '))
            else
                println(lmargin, l[1])
                TextWrap.println_wrapped(l[2], twopts_block2)
            end
        end
        println()
    end
...
function parse_args(arg_parser::ArgumentParser, args_list::Vector)
    try
        parsed_args = parse_args_unhandled(arg_parser, args_list)
    catch err
...
function parse_args_unhandled(arg_parser::ArgumentParser, args_list::Vector)
    ...
    if arg_parser.add_version
        ...
        add_argument(arg_parser, "--version", {"action" => show_version, "help" => "show version information and exit"})
        version_added = true
    end
    if arg_parser.add_help
        arg_parser.add_help = false
        add_argument(arg_parser, ["--help","-h"], {"action" => show_help, "help" => "show this help message and exit"})
        help_added = true
    end
    ...
    for f in arg_parser.args
        ...
        out_dict[f.dest_name] = deep_copy(f.default)
    end
